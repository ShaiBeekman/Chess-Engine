 ...
 * Milestone 50.
 *
 * Solve KNPK in both same-side color orientations with the generic Tier-1
 ...
 *
 * No persistence or runtime-routing changes are made here.  KNPK is promoted
 * to a runtime asset only after this exact-validation gate passes.
 */
public final class FourPieceTierOneKnpkExactValidationMain {

    private static final int DEFAULT_BELLMAN_SAMPLES =
            1_000_000;

    private FourPieceTierOneKnpkExactValidationMain() {
    }
    ...
    public static void main(
        ...
        if (args.length > 1) {
            throw new IllegalArgumentException(
                    "Usage: FourPieceTierOneKnpkExactValidationMain [bellman-samples]"
            );
        }
        ...
        System.out.println(
                "KNPK exact solve / validation / color-symmetry gate"
        );
        System.out.println(
                "=================================================="
        );
        ...
        System.out.println();
        System.out.println(
                "KNPK EXACT SOLVE / VALIDATION / COLOR-SYMMETRY GATE PASSED"
        );
        System.out.println(
                "NEXT: PERSIST KNPK AND VERIFY GENERIC TIER-1 RUNTIME ROUTING"
        );
    }
        ...
        for (int state = 0;
            ...
            if (whiteOutcome
                ...
                throw new IllegalStateException(
                        "KNPK color-symmetry outcome mismatch: "
                                + describe(
                                state
                        )
                                + " <-> "
            ...
            if (white.distance()[state]
                ...
                throw new IllegalStateException(
                        "KNPK color-symmetry DTM mismatch: "
                                + describe(
                                state
                        )
                                + " <-> "
    ...
    private static BellmanCounts validateBellman(
        ...
        while (checked
             ...
             * legal children remain inside KNPK.
             */
            if (external) {
                continue;
            }
    ...
    private static void fail(
        ...
        throw new IllegalStateException(
                "KNPK Bellman validation failed: "
                        + why
                        + "\n  state: "
                        + describe(
                        state
